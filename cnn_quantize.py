...
if __name__ == '__main__':
    ...
    if not os.path.isdir(f"./{dataset}_{appliance}"):
        ...
        dissagregator = CNNDissagregator(appliance, dataset, 1, 256)

        pruned_model = tf.keras.models.load_model(
            f'./{dataset}/saved_pruned_model/cnn_seq2seq/model_appliance_{appliance}_percentage_{percentage}')

        dissagregator.quantize(pruned_model)

        tracker = EmissionsTracker(output_file=f"./{dataset}/{dissagregator.model_name}_{appliance}_emission.csv")
        tracker.start()

        predicted_output_sequence = dissagregator.predict_quantized(X_test)
        ...
        dissagregator.compute_metrics(predicted_output_sequence, Y_test[:len(predicted_output_sequence)], state_test[:len(predicted_output_sequence)])
        print('*************************')
